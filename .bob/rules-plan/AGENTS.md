...
- **Alembic is the only schema management path** — `create_all()` exists in `database.py` for test/dev only. All new models must add an Alembic migration AND be imported in `migrations/env.py`.
...
- **Provenance is required for every processed output** — `app.ml.ingestion.provenance.write_provenance()` must generate a `.provenance.json` sidecar alongside every Parquet file written. This is a data governance constraint, not optional.
...
- **`src/frontend/` has no state management library** — auth state is read directly from `localStorage` via `lib/api.js`. Adding global state requires a new Context provider in `layout.jsx`.
...
- **CORS with wildcard `*` disables credentials** — `main.py` detects `cors_origin_list == ["*"]` and sets `allow_credentials=False` automatically to prevent browser rejection.

      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 pt-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={load} className="text-[#566b5c] hover:text-[#122018] dark:hover:text-slate-100 p-1 transition-colors" title="Refresh">
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin text-[#1e4d35]' : ''}`} />
            </button>
            </div>
          </div>
        </div>
        ...
        {showCreate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4">
            <form onSubmit={submitAsset} className="w-full max-w-lg dashboard-card-shape rounded-2xl p-6 space-y-4">
              ...
              <input required placeholder="Asset code" value={form.asset_code} onChange={(e) => setForm({ ...form, asset_code: e.target.value })} className="w-full px-3 py-2 text-xs font-mono bg-white dark:bg-slate-950/60 border border-[#1e4d35]/20 rounded-lg text-[#122018] dark:text-slate-100" />
              <select value={form.asset_type} onChange={(e) => setForm({ ...form, asset_type: e.target.value })} className="w-full px-3 py-2 text-xs font-mono bg-white dark:bg-slate-950/60 border border-[#1e4d35]/20 rounded-lg text-[#122018] dark:text-slate-100"><option>HELICOPTER</option><option>FIXED_WING</option><option>GROUND_VEHICLE</option><option>UAV</option></select>
              {['call_sign', 'manufacturer', 'model_number'].map((field) => <input key={field} placeholder={field.replace('_', ' ')} value={form[field]} onChange={(e) => setForm({ ...form, [field]: e.target.value })} className="w-full px-3 py-2 text-xs font-mono bg-white dark:bg-slate-950/60 border border-[#1e4d35]/20 rounded-lg text-[#122018] dark:text-slate-100" />)}
              <button disabled={saving} className="w-full px-4 py-2.5 text-xs font-mono font-bold bg-[#1e4d35] hover:bg-[#163a26] text-white rounded-xl disabled:opacity-50">{saving ? 'Registering...' : 'Register craft'}</button>
            </form>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="dashboard-card-shape p-4 rounded-2xl flex items-center justify-between">
            <div>
              <p className="text-xs font-mono text-[#566b5c] dark:text-slate-400">Total Fleet Assets</p>
              <p className="text-2xl font-extrabold font-mono text-[#122018] dark:text-slate-100 mt-1">{assets.length}</p>
            </div>
          ...
          <div className="dashboard-card-shape p-4 rounded-2xl flex items-center justify-between">
            <div>
              <p className="text-xs font-mono text-[#566b5c] dark:text-slate-400">Ready</p>
              <p className="text-2xl font-extrabold font-mono text-emerald-600 dark:text-emerald-400 mt-1">{readyCount}</p>
            </div>
          ...
          <div className="dashboard-card-shape p-4 rounded-2xl flex items-center justify-between">
            <div>
              <p className="text-xs font-mono text-[#566b5c] dark:text-slate-400">At Risk</p>
              <p className="text-2xl font-extrabold font-mono text-amber-600 dark:text-amber-400 mt-1">{atRiskCount}</p>
            </div>
          ...
          <div className="dashboard-card-shape p-4 rounded-2xl flex items-center justify-between">
            <div>
              <p className="text-xs font-mono text-[#566b5c] dark:text-slate-400">Not Ready</p>
              <p className="text-2xl font-extrabold font-mono text-red-600 dark:text-red-400 mt-1">{notReadyCount}</p>
            </div>
            <div className="w-10 h-10 rounded-full bg-red-500/10 border border-red-500/30 flex items-center justify-center text-red-600 dark:text-red-400">
        ...
        <div className="dashboard-card-shape rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4">
          <div className="relative flex-1 min-w-[260px]">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-xs font-mono bg-white dark:bg-slate-950/60 border border-[#1e4d35]/20 rounded-xl text-[#122018] dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-[#1e4d35] transition-colors"
            />
          </div>
          <select
            value={statusFilter}
        ...
        <div className="dashboard-card-shape rounded-2xl overflow-hidden p-6">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1e4d35]/20 text-xs font-mono text-[#566b5c] dark:text-slate-400 uppercase bg-[#e1eadf]/40 dark:bg-slate-950/40">
            ...
            <tbody className="divide-y divide-[#1e4d35]/15 dark:divide-slate-800/50">
              {loading ? (
                [...Array(8)].map((_, i) => (
                  <tr key={i}>
                    <td colSpan={7} className="px-4 py-3.5">
                      <div className="h-6 bg-[#e1eadf]/60 rounded animate-pulse" />
                    </td>
                  </tr>
                ))
              ) : filtered.length === 0 ? (
                ...
                filtered.map((asset) => {
                  ...
                  return (
                    <tr
                      key={asset.id}
                      className="hover:bg-[#e1eadf]/50 dark:hover:bg-slate-900/60 cursor-pointer transition-colors"
                      onClick={() => router.push(`/assets/${asset.id}`)}
                    >
                      <td className="px-4 py-3.5 font-mono font-bold text-[#122018] dark:text-slate-100 flex items-center gap-2.5">
                        <Plane className="w-4 h-4 text-[#1e4d35] dark:text-emerald-400" />
                        {asset.asset_code}
                      </td>
                      <td className="px-4 py-3.5 text-[#566b5c] dark:text-slate-300 hidden sm:table-cell">{asset.asset_type}</td>
                      <td className="px-4 py-3.5 text-[#566b5c] dark:text-slate-400 hidden md:table-cell font-mono">{asset.call_sign || '—'}</td>
                      <td className="px-4 py-3.5 text-[#122018] dark:text-slate-300 hidden lg:table-cell font-mono">{asset.total_hours?.toFixed(0) ?? '1420'} h</td>
                      <td className="px-4 py-3.5"><StatusBadge status={status} /></td>
                      <td className="px-4 py-3.5 font-mono text-[#566b5c] dark:text-slate-400 hidden sm:table-cell">
                        {rec?.confidence != null ? `${Math.round(rec.confidence * 100)}%` : '—'}
                      </td>
                      <td className="px-4 py-3.5 text-slate-400"><ChevronRight className="w-4 h-4" /></td>
                    </tr>
                  );
                })
              )}
